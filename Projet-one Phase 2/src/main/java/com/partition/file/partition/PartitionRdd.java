...
public class PartitionRdd {
	static private final int nsamples=10000;  //Je prévois de remplacer nsamples par astroDF.sample(..)
	...
	.write()
	.format("json")
 	.save("/home/vaadl/Bureau/test/astroDF.json");
	//astroDF.show();
	Dataset<Row>  astroDFparquet=spark.sql("SELECT * FROM json.`/home/vaadl/Bureau/test/astroDF.json`");
	//astroDFparquet.show();
...
/** Partie II: Partitionnement Intelligent
 * test_neighbours() teste les cellules voisines
 * 
 */
    int nrows = (int) astroDF.count(); 
    System.out.printf("%d", nrows);
    
    Neighbours test_neighbours = new Neighbours() {
		...
		public void test_neighbours(){
		    ...
		    for (long nside=12; nside<(1L<<omax); nside+=nside/2+1) {
		      ...
		      for (int m=0; m<nsamples; ++m){
		          ...
		          if (nb[n]>=0){
		            ++nnb;
		            assertTrue("erreur",base.pix2vec(nb[n]).angle(v)<maxang);
		          }
		    	  assertTrue("erreur",nnb>=7);
